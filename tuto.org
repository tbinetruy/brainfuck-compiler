...
We can now make use of ~vm//set-pc~ to implement ~vm//increment-pc~ which will increment our program counter, this function speaks for itself:
...
Finally, we can start compiling our Brainfuck instruction subset. The first thing we need is a /lexer/, which will tokenize Brainfuck code. It is a very simple lexer that just take a string with no spaces and ouputs a list of characters making up the string:
...
Incrementing a value in the heap pointer to by the heap pointer (stored in ~ecx~) is a little more complex. We first store the heap value contained in ~ecx~ to ~eax~ and call ~READ_RAM~ to read the heap value at ~ecx~. This pushes the heap value on the stack allowing us to increment it by adding unity to it. We then store this new value by saving it to ~ebx~, moving the heap pointer to ~eax~ again, and calling ~WRITE_RAM~.
